...
class Tabu_Search():
    ...
    def tabu_search(self):
        ...
        for j in range(0, 3):
            ...
            if self.new_roads not in tabu_list and (
                    self.new_fitness > self.current_fitness or self.road_points in tabu_list):
                self.road_points = self.new_roads
                self.current_fitness = self.new_fitness

        if self.road_points not in tabu_list:
            tabu_list.append(self.road_points)
